...
namespace UnityEngine.UI.Extensions
{
    ...
    public class SegmentedControl : UIBehaviour
    {
        ...
        {
            get { return m_allowSwitchingOff; }
        ...
        private void SetSelected(int value, bool notify = true)
        {
            ...
            if (selectedSegment)
            {
                ...
                if (segment)
                {
                    segment.SetSelected(false, notify);
                }
            ...
            if (value != -1)
            {
                ...
                if (segment)
                {
                    segment.SetSelected(true, notify);
                }
            }
        }

        public void SetSelectedSegmentWithoutNotify(int value)
        {
        ...
        IEnumerator DelayedInit()
        {
            ...
            if (m_selectedSegmentIndex != -1)
                SetSelectedSegmentWithoutNotify(m_selectedSegmentIndex);
        }
        ...
        static internal Sprite CutSprite(Sprite sprite, bool leftmost, bool rightmost)
        {
            ...
            return Sprite.Create(sprite.texture, rect, sprite.pivot, sprite.pixelsPerUnit, 0, SpriteMeshType.FullRect,
                border);
        }
        ...
        public void LayoutSegments()
        {
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                ...
                if (separator && i > 0)
                {
                    ...
                    Graphic sep = (sepTransform != null)
                        ? sepTransform.GetComponent<Graphic>()
                        : (GameObject.Instantiate(separator.gameObject) as GameObject).GetComponent<Graphic>();
                    ...
                    sep.rectTransform.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, insetX - SeparatorWidth,
                        SeparatorWidth);
