...
namespace Competition
{
    ...
    public class EventResultsProcessor : ResultsProcessor
    {
        ...
        public override List<int> GetFinalResultsWithCompetitorsList(List<int> competitorsList)
        {
            ...
            for (int i = 0; i < this.eventResults.allroundResults.Count; i++)
            {
                int localId = this.eventResults.allroundResults[i];
                int globalId = this.eventResults.competitorIds[localId];
    ...
    public class ClassificationResultsProcessor : ResultsProcessor
    {
        ...
        public override List<int> GetFinalResultsWithCompetitorsList(List<int> competitorsList)
        {
            return competitorsList.Select((val, ind) => (val, ind))
                .OrderByDescending(item => (this.classificationResults.totalResults[competitorsList[item.ind]], item.ind))
