...
public interface IJumpersMenuView
{
    ...
    bool FreezeJumperInfoCallbacks { get; set; }
...
public class JumpersMenuView : MonoBehaviour, IJumpersMenuView
{
    ...
    public Competitor SelectedJumper { get; private set; }
    ...
    #endregion
    public bool FreezeJumperInfoCallbacks { get; set; }
    ...
    private void Start()
    {
        toggleGroup.onValueChanged += (val) => { SelectedJumper = jumpers[val]; OnSelectionChanged?.Invoke(); };
    ...
    {
        if (!FreezeJumperInfoCallbacks) { OnCurrentJumperChanged?.Invoke(); }
    }
    ...
    public void LoadImage(string path) => StartCoroutine(imageCacher.GetSpriteAsync(path, SetJumperImage));
